...
float test_auto = 120.0f;         //测试使用

float catch_openAngle = -2800.0f; //接球包胶易磨损，采用此变量可动态调整，后续可封装进类
...
void Chassis_Task(void *pvParameters)
{
    ...
    {   
        ...
        /*用于测试*/
        //由于杨哥代码中对vesc读数的Bug,暂时只能这样调用数据进行测试
        //最好的解决方法是自己重写
      ...
      if(xQueueReceive(Chassia_Port, &ctrl, pdTRUE) == pdPASS)
      {
        ...
        /*==底盘控制==*/
           ...
           {
                ctrl.twist.linear.x = ctrl.twist.linear.x * (2.7 / 3.7) ;
                ctrl.twist.linear.y = ctrl.twist.linear.y * (2.7 / 3.7) ;
                ctrl.twist.angular.z = ctrl.twist.angular.z * 0.7;
                chassis.Control(ctrl.twist);
           }
...
//               chassis.Control(twist);
           ...
           /*===================================================================*/
            else if(ctrl.robot_crtl == BALL_MODE)
            {
                ...
                else if(ctrl.dribble_ctrl == DRIBBLE_ON)
