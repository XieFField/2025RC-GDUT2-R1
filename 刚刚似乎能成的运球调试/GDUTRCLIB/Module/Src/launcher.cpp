...
void Launcher::DribbleControl(bool shoot_ready, bool friction_ready, float shoot_speed)
{
	if(machine_init_)
    {
        update_timeStamp();
        ...
        if(friction_ready)
        {
            ...
            FrictionMotor[1].Out = -shoot_speed * left;
            FrictionMotor[2].Out = shoot_speed * right;
            FrictionMotor[0].Out = shoot_speed * mid;
            // 启动计时器（仅启动一次）
...
void Launcher::ShootControl(bool shoot_ready, bool friction_ready, float shoot_speed)
{
    if(machine_init_)
    {
        ...
        if(friction_ready)
        {
            ...
            FrictionMotor[1].Out = -shoot_speed * 1.08;
            FrictionMotor[2].Out = shoot_speed ;
            FrictionMotor[0].Out = shoot_speed * 0.85;
            // 启动计时器（仅启动一次）
            if (!friction_timer_started)
        ...
        if(shoot_ready && friction_ready)
        {
            if (xTaskGetTickCount() - friction_start_tick >= pdMS_TO_TICKS(1800))
            {
                PidPushSpd.target = PushPlanner.Plan(0,-1200,LauncherMotor[1].get_angle());
                PidPushSpd.current = LauncherMotor[1].get_speed();
                LauncherMotor[1].Out = PidPushSpd.Adjust();
            }
        }
        ...
        {
            PidPushSpd.target = PushPlanner.Plan(-1200,0,LauncherMotor[1].get_angle());
            PidPushSpd.current = LauncherMotor[1].get_speed();
            LauncherMotor[1].Out = PidPushSpd.Adjust();
        }
    }
