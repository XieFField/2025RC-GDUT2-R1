...
{
    uint8_t tmp;
    static uint8_t is_packet = 0; // 0:普通数据，1:数据包
        ...
        if (g_uart_rx_frame.sta.len < (ATK_MW1278D_UART_RX_BUF_SIZE - 1))   /* 判断UART接收缓冲是否溢出
            ...
            if (g_uart_rx_frame.sta.len == 0)                               /* 如果是一帧的第一个数据 */
            {
                ...
                __HAL_TIM_ENABLE(&g_tim_handle);                            /* 开启Timer计数 */
			            ...
			            if (!is_packet) // 普通数据处理逻辑
            {
                if (tmp == 0x55) // 检测到可能的数据包帧头1
                {
                    is_packet = 1; // 切换到数据包处理模式
            ...
            {
                if (g_uart_rx_frame.sta.len == 1)
                {
                    if (tmp == 0xAA)                                        /* 检查是否为包头2 */
                    {
                        g_uart_rx_frame.buf[g_uart_rx_frame.sta.len++] = tmp;
                    }
                    else
                ...
                else if (g_uart_rx_frame.sta.len >= 2 && g_uart_rx_frame.sta.len < (2 + 8)) /* 接收两个浮点数数据部分 */
                {
                    g_uart_rx_frame.buf[g_uart_rx_frame.sta.len++] = tmp;
                }
                else if (g_uart_rx_frame.sta.len == (2 + 8))
                {
                    if (tmp == 0x0D)                                        /* 检查是否为包尾1 */
                    {
                        g_uart_rx_frame.buf[g_uart_rx_frame.sta.len++] = tmp;
                    }
                    else
                ...
                else if (g_uart_rx_frame.sta.len == (2 + 8 + 1))
                {
                    if (tmp == 0x0A)                                        /* 检查是否为包尾2 */
                    {
                        g_uart_rx_frame.buf[g_uart_rx_frame.sta.len++] = tmp;
                        ...
                        /* 禁用定时器 */
                        __HAL_TIM_DISABLE_IT(&g_tim_handle, TIM_IT_UPDATE);
                        __HAL_TIM_DISABLE(&g_tim_handle);
