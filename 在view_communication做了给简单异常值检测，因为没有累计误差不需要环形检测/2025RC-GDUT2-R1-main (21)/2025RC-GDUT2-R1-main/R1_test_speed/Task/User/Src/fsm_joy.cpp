...
SpeedAction speed_control(HOOP_X, HOOP_Y);

SITUATION_E situation_ctrl;
                    ...
                    if(_tool_Abs(air_joy.SWA - 1000) < 50) //SWA UP
                    {
                        ...
                        else if(_tool_Abs(air_joy.SWD - 2000) < 50)
                        {
                           // ChassisYaw_Control(LASER_CALIBRA_YAW,&ctrl.twist.angular.z);  //用于锁定角度
                            ...
                            situation_ctrl = CLOCK_LASER;
                            speed_control.calc_error(situation_ctrl, &ctrl.twist.angular.z);
