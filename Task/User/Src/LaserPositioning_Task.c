...
void LaserPositioning_Task(void* argument)
{
	...
	float Yaw = (3.0 / 2.0) * PI;					// 偏航角变量，单位弧度，0表示世界坐标系正X轴方向，逆时针为正方向，范围是-PI到PI之间
...
//	}
	...
	//	}
	//}
	static uint8_t Laser_Data = 0x00; //初始化
	for(;;)
	{
...
static void LaserPositioning_XYWorldCoordinatesCalculate(WorldXYCoordinatesTypedef* WorldXYCoordinates, float Yaw, uint32_t FrontLaser, uint32_t RightLaser)
{
	...
	WorldXYCoordinates->Y = -(((float)FrontLaser * sin(Yaw - FrontLaserAngleOffset)) / 1000.0f);
	WorldXYCoordinates->X = -(((float)RightLaser * sin(Yaw - RightLaserAngleOffset)) / 1000.0f);
}
