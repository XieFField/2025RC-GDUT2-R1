...
// 声明外部 PID 控制器实例
extern PID_T point_X_pid;  // X轴位置PID控制器
extern PID_T point_Y_pid;  // Y轴位置PID控制器
Vector2D target_point;     // 目标点坐标
extern PID_T yaw_pid;      // 偏航角PID控制器
...
void mode_3(float *robot_vel_x, float *robot_vel_y) {
    ...
    if (!initialized) {          // 如果还没有初始化
        ...
        locate_init();           // 调用初始化函数
	//用于控制目标角度的角速度pid
	pid_param_init(&yaw_pid, PID_Position, 0.2, 0.0f, 0, 2.0f, 360.0f, 0.1f, 0.0f, 0.001f);
	
	//用于控制半径大小的法向速度pid
    pid_param_init(&point_X_pid, PID_Position, 1.0, 0.0f, 0, 0.1f, 180.0f,0.5f, 0.0f, 0.66f);
        initialized = 1;         // 标记为已初始化
    }
    
    // 集成多环半径设置（如果需要动态更新）
    set_multi_radius_rings();   // 设置多环半径（可考虑移到初始化中）
    ...
    if(_tool_Abs(vx) < 1e-6){    // 如果X轴输入接近零
            float angle_error = center_heading - ROBOT_REAL_POS_DATA.world_w;  // 计算角度误差
