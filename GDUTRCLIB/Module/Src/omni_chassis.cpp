...
void Omni_Chassis::Velocity_Calculate(Robot_Twist_t cmd_vel)
{
    ...
    if (is_yaw_initialized) {
        relative_yaw = ROBOT_REAL_POS_DATA.world_w - yaw_init;
        // 角度归一化到 [-180, 180] 度
    ...
    float COS, SIN;
    COS = cos(relative_yaw * PI / 180);
    SIN = sin(relative_yaw * PI / 180);

    // ----------- 世界坐标系速度转换为机器人坐标系速度 -----------
    // 注意：这里假设cmd_vel是世界坐标系下的速度命令
    float temp_x = cmd_vel.linear.x;
    float temp_y = cmd_vel.linear.y;
    cmd_vel.linear.x = temp_x * COS - temp_y * SIN;  // 修正坐标变换公式
...
bool Omni_Chassis::Pid_Mode_Init(int num, float LowPass_error, float LowPass_d_err, bool D_of_Current, bool Imcreatement_of_Out)
{
 ...
 //添加轮子速度逆向解算函数
void Omni_Chassis::wheel_speed_to_chassis_vel(float *vx, float *vy, float *wz) 
{
    // 获取当前轮子速度并转换为m/s
    float w0 = WheelMotor[0].get_speed() / ChassisVel_Trans_MotorRPM(Wheel_Radius, 19);
    float w1 = WheelMotor[1].get_speed() / ChassisVel_Trans_MotorRPM(Wheel_Radius, 19);
    float w2 = WheelMotor[2].get_speed() / ChassisVel_Trans_MotorRPM(Wheel_Radius, 19);
    
    // 逆向解算矩阵
    *vx = (2*w0 - w1 - w2) / 3.0f;
    *vy = (w2*COS30 - w1*SIN60) / SIN30;
    *wz = (w0 + w1 + w2) / (3.0f * Chassis_Radius);
}
